...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains only elements present in the two sets") {
    new TestSets {
  ...
  test("forall") {
    new TestSets {
      val set1 = union(union(s1, s2), s3)
      val b1 = forall(set1, (x: Int) => x < 2)
      assert(!b1, "Forall 1")
      val b2 = forall(set1, (x: Int) => x > 0)
      assert(b2, "Forall 2")
    }
  }
  ...
  test("exists") {
    new TestSets {
      val set1 = union(union(s1, s2), s3)
      val b1 = exists(set1, (x: Int) => x < 0)
      assert(!b1, "Exists 1")
      val b2 = exists(set1, (x: Int) => x == 2)
      assert(b2, "Exists 2")
    }
  }
